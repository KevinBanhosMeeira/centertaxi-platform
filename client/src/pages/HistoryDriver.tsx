...
export default function HistoryDriver() {
  ...
  const { data: rides, isLoading } = trpc.rides.getHistoryDriver.useQuery();
  ...
  const totalEarnings = completedRides.reduce((sum, ride) => sum + parseFloat(ride.priceEstimate || "0"), 0);
          ...
          {completedRides.length > 0 && (
            <Card className="p-6 bg-card border-border">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-sm text-muted-foreground">Corridas concluídas</div>
                  <div className="text-2xl font-bold text-foreground">{completedRides.length}</div>
                </div>
                <div>
                  <div className="text-sm text-muted-foreground">Total ganho</div>
                  <div className="text-2xl font-bold text-primary">R$ {totalEarnings.toFixed(2)}</div>
                </div>
              </div>
            </Card>
          )}
